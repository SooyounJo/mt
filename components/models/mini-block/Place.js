...
const Place = ({ visible = true, onDragStart, onDragEnd, onDrop, selectedNumber = null }) => {
  ...
  const originalPosition = [1.1, -1, -1.2];
  ...
  const handlePointerEnter = (e) => {
    e.stopPropagation();
    if (visible && !isOnLP && selectedNumber === 2) {
      setIsDraggable(true);
  ...
  const handlePointerDown = (e) => {
    if (selectedNumber !== 2 || isOnLP) return;
