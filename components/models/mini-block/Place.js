...
export default function Place({ visible = true, positions, dragEnabled = true, onDragStart, onDragEnd }) {
  ...
  return (
    <group>
      {files.map((file, idx) => {
        ...
        return (
          <group key={file} position={onLP[idx] ? [lpPosition[0], lpPosition[1] + 0.1, lpPosition[2]] : posArr[idx]}>
            {/* 실제 모델 */}
            <primitive
